...
- Bump `rustls-webpki` to 0.103.13, clearing RUSTSEC-2026-0104 (a reachable
  panic when parsing certificate revocation lists). pngx does not parse CRLs,
  so it was not exploitable; the bump keeps `cargo audit` clean.

## [0.9.0] - 2026-05-20

### Added

- Add `pngx documents notes <id>` to list a document's notes, and
  `pngx documents add-note <id> "<text>"` / `pngx documents remove-note <id>
  <note-id>` to add and remove them.
- Add MCP tools `documents_notes`, `documents_add_note`, and
  `documents_delete_note` mirroring the new note commands.
